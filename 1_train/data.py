...
class DataBowl3Detector:  # dataset
    def __init__(self, config, process='train', split_comber=None):
        # assert (process == 'train' or process == 'val' or process == 'test')
        ...
        all_image_name = self.all_filename_list()
        if process != 'test':
            all_image_name = [f for f in all_image_name if (f not in self.blacklist)]
        self.filenames = [os.path.join(self.data_dir, '%s_clean.npy' % idx) for idx in all_image_name]

        if self.process != 'test':
            labels = []
            for idx in all_image_name:
                l = np.load(os.path.join(self.data_dir, '%s_label.npy' % idx))
                if np.all(l == 0):
                    l = np.array([])
                labels.append(l)
            ...
            for i, l in enumerate(labels):
                if len(l) > 0:
                    for t in l:
                        if t[3] > sizelim:
                            self.bboxes.append([np.concatenate(
                                [[i], t])])
                        if t[3] > sizelim2:
        ...
        self.label_mapping = LabelMapping(config, self.process)
    ...
    def __getitem__(self, idx, split=None):
        ...
        if self.process != 'test':
            if not is_random_img:
                ...
                isScale = self.augtype['scale'] and (self.process == 'train') or (self.process == 'all')
                sample, target, bboxes, coord = self.crop(imgs, bbox[1:], bboxes, isScale, is_random)
                if self.process == 'train' or self.process == 'all' and not is_random:
            ...
            return np.asarray(sample, dtype=np.float32), label.transpose(3, 4, 0, 1, 2), \
            ...
            return np.asarray(imgs, dtype=np.float32), np.asarray(coord2, dtype=np.float32), \
                   np.array(nzhw,dtype=np.float32)
    ...
    def all_name_path(self, file_suffix="clean.npy"):
...
# Calculate jump, rotate, mirrored data
    ...
    def __call__(self, imgs, target, bboxes, isScale=False, isRand=False):
        ...
        for i in range(3):
            if not isRand:  # True
                ...
                s = np.max([imgs.shape[i + 1] - crop_size[i] * 4 / 5, imgs.shape[i + 1] / 2 + bound_size])
                # e = np.min([crop_size[i] / 2, imgs.shape[i + 1] / 2 - bound_size])   # change !
                e = int(-crop_size[i] / 4)
                target = np.array([np.nan, np.nan, np.nan, np.nan])
            if s > e:
                start.append(np.random.randint(e, s))  # !
                # print(1, 's:', s, 'ce', e, 'start', start)
            else:
...
class LabelMapping(object):
    ...
    def __call__(self, input_size, target, bboxes):
        ...
        oz = np.arange(offset, offset + stride * (output_size[0] - 1) + 1, stride)  # 1.5 -- 125.5  32
        oh = np.arange(offset, offset + stride * (output_size[1] - 1) + 1, stride)  # 1.5 -- 125.5  32
        ow = np.arange(offset, offset + stride * (output_size[2] - 1) + 1, stride)  # 1.5 -- 125.5  32
        ...
        if self.phase == 'train' or self.phase == 'all' and self.num_neg > 0:
        ...
        dd = np.log(target[3] / anchors[pos[3]])  # exp
